...
class Graphic extends StatefulWidget {
  ...
  static List<Recording> recList = [
    ...
    Recording(
        ...
        clientId: 1,
        masterId: 0,
        dateRec: DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 12, 30),
        procId: 1,
        note: "Note 2"
    )
  ];
  ...
  @override
  State<StatefulWidget> createState() => _graphicState();
          ...
          children: [
            ...
            Expanded(
              child: ListView.builder(
                itemCount: Graphic.recList.length, // Замените на реальное количество элементов
                itemBuilder: (context, index) {
                  Recording rec = Graphic.recList[index];
                  User client = Graphic.clientList[rec.clientId];
                  Procedure procedure = Graphic.procList[rec.procId];
                  return Column(

                    children: [
                      ListTile(
                        trailing: Image.asset("assets/images/pen.png"),
                        leading: Text("${DateFormat("H:m").format(rec.dateRec)} - ${DateFormat("H:m").format(rec.dateRec.add(Duration(minutes: procedure.minute)))}",
                          style: TextStyle(
                              fontFamily: "Nunito",
                              fontWeight: FontWeight.w700,
                              fontSize: 16
